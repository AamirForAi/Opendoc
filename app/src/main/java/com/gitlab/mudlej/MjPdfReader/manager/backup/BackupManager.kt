...
class BackupManager(
    ...
    suspend fun export(uri: Uri, includePasswords: Boolean): ExportSummary = withContext(Dispatchers.IO) {
        ...
        val records = databaseManager.findAllRecords().map { it.toBackup(includePasswords) }
        val bookmarks = databaseManager.findAllUserBookmarks().map { it.toBackup() }
        val data = BackupData(
    ...
    private suspend fun mergeRecords(records: List<BackupPdfRecord>): Pair<Int, Int> {
        var inserted = 0
        var updated = 0
        val existingByHash = databaseManager.findAllRecords().associateBy { it.hash }
        ...
        if (toSave.isNotEmpty()) {
            databaseManager.upsertRecords(toSave)
        }
        return inserted to updated
    }

    private suspend fun mergeBookmarks(bookmarks: List<BackupUserBookmark>): Int {
        val existingByKey = databaseManager.findAllUserBookmarks().associateBy { it.fileHash to it.pageIndex }
        ...
        if (toSave.isNotEmpty()) {
            databaseManager.upsertUserBookmarks(toSave)
        }
        return toSave.size
    }
