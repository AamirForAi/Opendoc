...
class DocumentLoader(
    ...
    fun initPdf(uri: Uri) {
        ...
        scope.launch {
            val hash = computeHash(context, doc)
    ...
    fun initPdfViewAndLoad(viewConfigurator: Configurator, savePassword: Boolean = false) {
        ...
        scope.launch {
            val hash = doc.fileHash ?: computeHash(context, doc)
    ...
    private suspend fun findCachedCropMargins(fileHash: String?): CropMargins? {
        ...
        val storedCropMargins = databaseManager.findCropMargins(fileHash, CropMargins.STORAGE_VERSION)
            ?: return null
    ...
    private fun createPdfRecord(
        ...
        scope.launch {
            ...
            if (doc.fileHash == null && expectedFileHash == null) {
                val computedHash = computeHash(context, doc)
                if (!vm.isCurrent(loadToken, documentUri)) {
                    return@launch
                }
            ...
            if (databaseManager.hasRecord(fileHash)) {
                ...
                doc.autoScrollSpeed?.let { databaseManager.setAutoScrollSpeed(fileHash, it) }
                if (!vm.isCurrent(loadToken, documentUri)) {
                    return@launch
                }
                ...
                val record = PdfRecord.from(fileHash, doc, password)
    ...
    private fun setCurrentPage(
        ...
        scope.launch {
            ...
            val hash = doc.fileHash ?: expectedFileHash ?: computeHash(context, doc)
            if (!vm.isCurrent(loadToken, documentUri)) {
                return@launch
            }
            if (hash != null) {
